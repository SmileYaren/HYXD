...
function mysql_connect_to_auth_center()
	...
	Mysql.connect(auth_conf.host,auth_conf.port,
		...
		Logger.debug("connect to auth center db sucess!!!")
		mysql_conn = conn
	end)
end
...
function get_guest_uinfo(g_key,ret_handler)
	...
	--print("begin query db:"..tostring(mysql_conn).."  "..tostring(sql_cmd))
	...
	--print("query db end:",tostring(#ret))
		if ret ==nil or #ret<=0 then --没有记录
			if ret_handler~=nil then
				ret_handler(nil,nil)
			end
			return
...
--升级游客账号 存储账号密码信息
function do_account_upgrade(uid,uname,upwd_md5,ret_handler)
	-- body
	if mysql_conn==nil then
		if ret_handler then
			ret_handler("mysql is not connected!",nil)
