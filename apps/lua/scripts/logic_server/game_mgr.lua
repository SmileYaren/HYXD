...
function do_load_robot_ugame_info()
	...
	mysql_game.get_robots_ugame_info(function (err,ret )
		...
		if not ret or #ret<0 then
			print("***get_robots_ugame_info  nil !***")
			return
		end
		...
		do_load_robot_uinfo(1,ret) --开始加载（redis）第一个 机器人
	end)
end
--递归 加载机器人
function do_load_robot_uinfo(now_index,robots)
	...
	mysql_center.get_uinfo_by_uid(robots[now_index].uid,function (err,uinfo)
		...
		if now_index>#robots then
			--所有的机器人都缓存到了redis_center中了
			do_new_robot_player(robots)
			return
		end
...
function do_new_robot_player(robots)
	-- body
	if # robots<=0 then
		return
	end
	...
	print("this is all robot "..#robots .."  has enter!!")
end
-----end
...
--没过一段事件 遍历一遍等待列表 将玩家添加到游戏房间中
function do_match_players()

	local zid, wait_list
	for zid, wait_list in pairs(zone_wait_list) do 
		local k, v
		for k, v in pairs(wait_list) do  --wait_list==k:index  v:player
			local match = search_inview_match_mgr(zid)--找个空闲房间
			if match then
				 if not match:enter_player(v) then 
				 	Logger.error("match system error : player state: ", v.state,"matchid:",math.matchid)
				 else
				 	wait_list[k] = nil
				 end
			end
...
function search_idle_robot(zid)
	...
	for k,v in pairs(robots) do
		if v.matchid ==-1 then --没有在任何一个比赛房间
			return v
		end
	end
	return nil
	...
	for zid,match_list in pairs(zone_match_list) do
		for k,match in pairs(match_list) do
			if match.state==State.InView then
				local robot =search_idle_robot(zid)--获取一个空闲的机器人
				if robot then 
...
function login_logic_server(s, req)
	...
	local stype = req[1]
	local body = req[4]
	print("set udp_ip and udp_port",body.udp_ip,body.udp_port)

	local p = logic_server_players[uid] -- player对象
	if p then -- 玩家对象已经存在了，更新一下session就可以了; 
		p:set_session(s)

		p:set_udp_addr(body.udp_ip,body.udp_port)--只有网关服务器 知道 用户的ip和port
...
function  enter_zone(s, req)
	...
	local p = logic_server_players[uid]
	-- print("~~~~~~enter_zone~~~~~~",p.zid,p.state)
...
function do_exit_mathch(s,req)
	...
	if not match or match.status~=State.InView then
		send_status(req[1],Cmd.eExitMatchRes,uid,Respones.InvalidOpt)
		return		
	end
	match:exit_player(p)
...
function do_udp_test(s,req)
	...
	local ctype = req[2]
	local body = req[4]
	print("~~~~~~~do_udp_test~~~~~~~~~",body.content)
	local msg = {
		stype,ctype,0,
		{
